...
export default function KeikoPromptsAdmin() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/keiko/packs`)
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/keiko/prompts/by-pack/${selectedPack}`)
      .then(({ data }) => setPrompts(data))
      .catch(console.error);
  }, [selectedPack]);

  const getUniqueValuesFromFixedOptions = (key) =>
    Array.from(new Set(prompts.flatMap(p => p.fixedOptions?.[key]?.map(opt => opt.label) || []))).sort();

  const platforms = Array.from(new Set(prompts.map(p => p.platform || ''))).filter(Boolean).sort();
  ...
  const displayed = prompts
    ...
    .filter(p => !filterEstilo || (p.fixedOptions?.estilo?.some(opt => opt.label === filterEstilo)))
    .filter(p => !filterTematica || (p.fixedOptions?.['temática']?.some(opt => opt.label === filterTematica)));
  ...
  // NUEVO: borrado masivo de los seleccionados
  const handleMassDelete = async () => {
    if (selectedIds.length === 0) return;
    if (!window.confirm(`Vas a borrar ${selectedIds.length} prompt(s). Esta acción no se puede deshacer. ¿Continuar?`)) {
      return;
    }
    try {
      // Opción A: DELETE con body (Axios permite data en delete)
      await axios.delete(`${process.env.REACT_APP_API_URL}/api/keiko/prompts/mass-delete`, {
        data: { ids: selectedIds }
      });

      // // Opción B (alternativa si tu backend prefiere POST):
      // await axios.post(`${process.env.REACT_APP_API_URL}/api/keiko/prompts/mass-delete`, {
      //   ids: selectedIds
      // });

      // Actualizamos la lista local
      const remaining = prompts.filter(p => !selectedIds.includes(p._id));
      setPrompts(remaining);
      setSelectedIds([]);
    } catch (err) {
      console.error(err);
      alert('No se pudieron borrar los prompts seleccionados.');
    }
  };
    ...
    <div className="keiko-admin-container">
      ...
      {selectedIds.length > 0 && (
        <div className="mass-edit-bar">
          ...
          {/* NUEVO: botón de borrado masivo */}
          <button
            onClick={handleMassDelete}
            className="danger-btn"
            style={{ marginLeft: '8px' }}
            title="Borrar todos los seleccionados"
          >
      ...
      <table className="keiko-admin-table">
        ...
        <tbody>
            ...
            <tr key={p._id}>
              <td>
                <input
                  ...
                  onChange={e => {
                    const checked = e.target.checked;
                    setSelectedIds(checked ? [...selectedIds, p._id] : selectedIds.filter(id => id !== p._id));
                  }}
                />
              </td>
              <td>{p.scene}</td>
          ...
          {displayed.length === 0 && (
            <tr>
              <td colSpan="8" style={{ textAlign: 'center', opacity: 0.7, padding: '1rem' }}>
                No hay prompts para los filtros actuales.
              </td>
            </tr>
          )}
        </tbody>
