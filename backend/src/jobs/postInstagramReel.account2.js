...
// ====== CREA VIDEO 9:16 CON FFmpeg (xfade entre imágenes) ======
async function buildReelVideoFromImages({
  images,
  perSlideSec = 4,
  profile = REEL_PROFILE,
  transitionSec = 0.8,       // duración del crossfade
  transitionType = 'fade'    // 'fade', 'smoothleft', 'smoothright', 'circleopen', etc.
}) {
  await ensureFfmpeg();
  ...
  // Ajusta duración para no pasar de 90s teniendo en cuenta el solapamiento del crossfade
  let dur = Math.max(MIN_PER_SLIDE_SECS, perSlideSec);
  let finalDur = n * dur - (n - 1) * transitionSec;
  if (finalDur > MAX_REEL_SECS) {
    dur = Math.max(MIN_PER_SLIDE_SECS, (MAX_REEL_SECS + (n - 1) * transitionSec) / n);
    finalDur = n * dur - (n - 1) * transitionSec;
  }
  ...
  // Inputs: cada imagen como stream loop con duración 'dur'
  const args = [];
  for (const fp of localImgs) {
    args.push('-loop', '1', '-t', String(dur), '-i', fp);
  }
  // Audio silencioso para el total final
  args.push('-f', 'lavfi', '-t', String(finalDur), '-i', 'anullsrc=r=48000:cl=stereo');

  // filter_complex:
  // 1) Preparar cada input a WxH, fps, yuv420p -> [v0]...[v{n-1}]
  const preps = [];
  for (let i = 0; i < n; i++) {
    preps.push(
      `[${i}:v]scale=${p.w}:-2:force_original_aspect_ratio=decrease,` +
      `pad=${p.w}:${p.h}:(${p.w}-iw)/2:(${p.h}-ih)/2:color=black,` +
      `setsar=1,fps=${FPS},format=yuv420p[v${i}]`
    );
  }

  // 2) Cadena de xfade secuencial
  // offset_k = (k*dur - k*transitionSec) para el k-ésimo xfade (k=1..n-1)
  const xfs = [];
  for (let k = 1; k <= n - 1; k++) {
    const inA = (k === 1) ? 'v0' : `x${k - 1}`;
    const inB = `v${k}`;
    const out = `x${k}`;
    const offset = (k * dur - k * transitionSec).toFixed(3);
    xfs.push(
      `[${inA}][${inB}]xfade=transition=${transitionType}:duration=${transitionSec}:offset=${offset},format=yuv420p[${out}]`
    );
  }
  const lastLabel = `x${n - 1}`; // salida de la última transición

  const filterComplex = `${preps.join(';')};${xfs.join(';')}`;
  ...
  args.push(
    ...
    // H.264 “ultra-compatible” (sin B-frames), CBR/ABR estable
    '-c:v', 'libx264',
    '-profile:v', 'main',
    '-level', '4.1',
    '-pix_fmt', 'yuv420p',
  ...
  // Limpieza
  try { for (const pth of localImgs) fs.unlink(pth, () => {}); } catch {}

  return { outPath, totalDur: finalDur };
}
...
// ====== JOB PRINCIPAL ======
/**
 * Construye y publica un Reel 9:16 con imágenes publishable:true y transiciones.
 *
 * @param {Object} opts
 * @param {number} [opts.limit=6]        - nº de imágenes (2..20)
 * @param {number} [opts.perSlideSec=4]  - seg por imagen (visible). Final = n*dur - (n-1)*transitionSec
...
export async function postReelAccount2({
  ...
  // 1) Construir vídeo (con transiciones)
  ...
  console.log(`[keikodevfree.reel] Vídeo generado ${outPath} (${totalDur.toFixed(2)}s)`);
