...
export const recibirPostbackCpx = async (req, res) => {
  ...
  try {
    const recompensa = await CpxRecompensa.findOne({ trans_id });
    ...
    if (parseInt(status) === 1) {
      ...
      if (!recompensa) {
        ...
        console.log(`✅ Créditos añadidos a ${user_id}: +${creditos}`);
    ...
    if (parseInt(status) === 2 && recompensa?.status === 1) {
      ...
      console.log(`↩️ Créditos revertidos a ${user_id}: -${recompensa.creditos_dados}`);
